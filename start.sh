...
if [ "${FW}" = "auto" ]; then
    ...
    elif [ -f "index.php" ] || [ -f "*.php" ] 2>/dev/null || ls *.php 2>/dev/null | head -1 | grep -q php; then FW="php"
...
# ===== ADMINER (auto deploy untuk PHP/Laravel) =====
_deploy_adminer() {
    if [ ! -f "/home/container/adminer.php" ]; then
        echo "Deploying Adminer..."
        curl -sL https://www.adminer.org/latest.php -o /home/container/adminer.php && \
...
case "${FW}" in
    ...
    php)
        ...
        echo " PHP: ${PHP_BIN}"
        ${PHP_BIN} -d session.save_path=/home/container/sessions \
    ...
    python)
        ...
        # Detect framework & run
        if grep -q "fastapi\|FastAPI" requirements.txt 2>/dev/null; then
        ...
        elif grep -q "flask\|Flask" requirements.txt 2>/dev/null; then
            echo "Running Flask..."
            FLASK_APP=${ENTRY} FLASK_RUN_HOST=0.0.0.0 FLASK_RUN_PORT=${SERVER_PORT} \
                python3 -m flask run

        elif grep -q "django\|Django" requirements.txt 2>/dev/null; then
            echo "Running Django..."
