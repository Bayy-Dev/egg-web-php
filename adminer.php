...
// Download adminer core ke /tmp
$adminer_core = sys_get_temp_dir() . "/adminer_core.php";
if (!file_exists($adminer_core) || (time() - filemtime($adminer_core)) > 86400) {
    $core = @file_get_contents("https://www.adminer.org/latest.php");
    if ($core) file_put_contents($adminer_core, $core);
}
if (!file_exists($adminer_core)) {
    die("<h2 style='color:red;font-family:monospace'>❌ Gagal load Adminer. Cek koneksi internet.</h2>");
}

// Custom class - HARUS sebelum require core
...
body{background:var(--bg)!important;background-image:radial-gradient(ellipse at 20% 50%,rgba(0,50,100,.3) 0,transparent 50%),radial-gradient(ellipse at 80% 20%,rgba(100,0,100,.2) 0,transparent 50%)!important;color:var(--ti)!important;font-family:"Share Tech Mono",monospace!important;min-height:100vh}
...
#menu a{color:var(--c)!important;font-family:"Share Tech Mono",monospace!important;font-size:13px!important;padding:6px 15px!important;display:block!important;border-left:2px solid transparent!important;transition:all .2s!important}
