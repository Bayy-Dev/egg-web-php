...
# ============================================
#   CEK & INSTALL PHP KALAU BELUM ADA
# ============================================
if ! command -v php &> /dev/null; then
    echo ">>> PHP tidak ditemukan, menginstall PHP..."
    apt-get update -qq 2>/dev/null
    apt-get install -y -qq php-cli php-curl php-mbstring php-xml php-zip php-mysql php-gd php-bcmath php-json 2>/dev/null
    echo ">>> PHP terinstall: $(php -v | head -1)"
fi

if ! command -v mysql &> /dev/null; then
    apt-get install -y -qq default-mysql-client 2>/dev/null
fi
